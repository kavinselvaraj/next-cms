...
- **✅ Resolved by reusing what already exists**: rather than building a new `FaqAccordion` variation, use `FaqQuestionList`'s existing `grid` variation (already live on this sandbox's own `faq` hub) — it's already exactly "heading + inline-wrapped topic links, plain navigation, no context." One instance per category, `items`: `question` (Text) / `href` (Text) grouped under a `heading`.
- **Trade-off versus a topic-key-aware hub slice**: `href` here is a free-text field — an editor authors it as a literal string like `/faq?topic=payment`. Nothing validates that `payment` is actually one of the 11 real topic keys `sidebar_nav`/`QuestionList`/`QuestionAnswer` use — a typo (`?topic=paymnet`) would silently link to a topic that matches nothing. Accept that risk for zero new code, or build a `Select`-backed hub variation later if typos become a real problem.
