...
function formatMigrationEvent(event: {
    ...
    const progress =
        event.data?.current && event.data?.total ? ` ${event.data.current}/${event.data.total}` : "";
...
function selectStructuredTextBlockType(single?: string) {
    ...
    return configuredTypes.find((type) => supportedBlockTypes.includes(type)) ?? "paragraph";
}
...
function getDocumentIdOverride(modelId: string, locale: string) {
    const key = ["PRISMIC", modelId.toUpperCase(), locale.toUpperCase(), "DOCUMENT_ID"].join("_");
