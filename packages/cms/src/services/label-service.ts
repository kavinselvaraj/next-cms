...
import { getPrismicDocuments, type PrismicLabelDocument } from "../prismic/document-registry";
...
export function createLabelService<TMessages extends Record<string, unknown>>(
    ...
    async function fetchPrismicLabels(locale: string): Promise<TMessages> {
        ...
        const parentDocument = await fetchParentDocument(client, options.parentDocumentType, lang);
        ...
        const childIds = expectedDocumentTypes
            .map((documentType: string) => findChildDocumentId(parentDocument.data, documentType))
    ...
    async function fetchParentDocument(
        ...
        try {
            ...
            logLabelFlow("prismic parent document locale lookup failed, retrying without lang", {
                parentDocumentType,
                lang,
            ...
            try {
                return (await client.getSingle(parentDocumentType as never)) as PrismicParentDocument;
            } catch {
                throw new Error(
...
export function resolvePrismicLabels<T extends Record<string, unknown>>(
    ...
    const resolvedEntries = Object.entries(template).map(([documentType, documentTemplate]) => {
...
function mapGroupItem(template: unknown, value: unknown, context: MappingContext): unknown {
...
function getObjectSource(source: unknown, context: MappingContext): Record<string, unknown> {
    if (isRecord(source)) {
        return source;
    }
...
function assignDeep(target: Record<string, unknown>, pathParts: string[], value: unknown) {
